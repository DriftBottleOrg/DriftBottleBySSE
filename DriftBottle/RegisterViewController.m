...
@property (strong, nonatomic)UserNameAndPassWord *userNameAndPassWord;
...
- (IBAction)Register:(id)sender {
    ...
    if([self.passWord isEqualToString:self.passWordAgain])
    {
        if(!self.userNameAndPassWord){
            self.userNameAndPassWord = [[UserNameAndPassWord alloc] init];
            self.userNameAndPassWord.userName = self.userName;
            self.userNameAndPassWord.passWord = self.passWord;
            
            //存储用户的用户名和密码
            [self storeUserNameAndPassWord:self.userNameAndPassWord];
...
- (void)storeUserNameAndPassWord:(UserNameAndPassWord *)userNameAndPassWord
{
